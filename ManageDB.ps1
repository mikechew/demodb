...
function Gen-Sample-ParamFile ()
{
  ...
  "[string] `$tgt_label = " + [char]34 + "mDB_jd_mount" + [char]34 + "      ## Label when mount/remount application " | Out-File $sampleFile -Append  -Encoding Ascii 
  ...
  [string]$tgtnodeip )
{
  ...
  $UseScript, $localScript = Build-PrePostScript
   ...
   if ( $UseScript ) {
     $cmd = "udstask mountimage -appid " + $appid + " -host " + $hostid + " -label " + $tgt_label + " -appaware " + $xmlstring + `
        " -script " + [char]34 + $localScript + [char]34 + " -nowait | Out-Null" 
   } else {
     $cmd = "udstask mountimage -appid " + $appid + " -host " + $hostid + " -label " + $tgt_label + " -appaware " + $xmlstring + " -nowait | Out-Null"
   }
...

{
  ...
  if ($cmdline -ne $Null) {         ## ####  udstask expireimage -nowait -image Image_123  
    ...
    if ( $UseScript ) {
      $cmd = $cmdline + " -host " + $hostid + " -label " + $tgt_label + " -appaware " + $xmlstring + `
            " -script " + [char]34 + $localScript + [char]34 + " -nowait | Out-Null" 
    } else {
      $cmd = $cmdline + " -host " + $hostid + " -label " + $tgt_label + " -appaware " + $xmlstring + " -nowait | Out-Null"
    }
...
if ($tgt_label -eq $null -or $tgt_label -eq "") {
  $tgt_label = "mDB-" + $action[0] + "-" + $vdpuser + "-" + $tgt_appname 
  }
